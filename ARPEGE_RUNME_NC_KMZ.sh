...
NomDuFichierMeteoFrance=`python3.9 PYTHON/RequeteArpege.py $1 SP1`
rc=$?
if ! [ $rc == 0 ]; then
...
DateDeLaPrevision=`python3.9 PYTHON/DateDeLaPrevisionArpege.py $1`
FinDeLaPrevision=`python3.9 PYTHON/FinDeLaPrevisionArpege.py $1`
...
if [ -e KML/IMAGES/$NomDuFichierMeteoFrance.nc.png ]; then
	...
	/Applications/ParaView-5.9.1.app/Contents/bin/pvpython --force-offscreen-rendering PYTHON/visu_arpege.py DATA/$NomDuFichierMeteoFrance.nc
...
echo
cat KML/arpege_templateSpanKMZ.kml | sed "s/ICILEFICHIER/$NomDuFichierMeteoFrance.nc.png/g" > tmp.kml
