...
NomDuFichierMeteoFrance=`python3.9 PYTHON/RequeteArpegeHD.py $1 SP1`
rc=$?
if ! [ $rc == 0 ]; then
...
DateDeLaPrevision=`python3.9 PYTHON/DateDeLaPrevisionArpegeHD.py $1`
FinDeLaPrevision=`python3.9 PYTHON/FinDeLaPrevisionArpegeHD.py $1`
DateDuRun=`python3.9 PYTHON/DateDuRunArpegeHD.py $1`
...
if [ -e KML/IMAGES/$NomDuFichierMeteoFrance.nc.png ]; then
	...
	/Applications/ParaView-5.9.1.app/Contents/bin/pvpython --force-offscreen-rendering PYTHON/visu_arpegehd.py DATA/$NomDuFichierMeteoFrance.nc
...
echo
cat KML/arpegehd_templateSpanKMZ.kml | sed "s/ICILEFICHIER/$NomDuFichierMeteoFrance.nc.png/g" > tmp.kml
